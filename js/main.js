...
var x=document.getElementById('id_p');
var bombs=[];
...
function createBomb() {
    ...
    while (!bombCreated) { //пока бомбу не создали
        ...
        // проверка на змейку, еду и бомбу.
        if (!bomb_cell_classes.includes('snake-unit','food-unit','bomb-unit')) {
            var classes = '';
...
function respawn() {
    ...
    // Голова змейки
    var snake_head = document.getElementsByClassName('cell-' + start_coord_y + '-' + start_coord_x)[0];//присвоили координаты.
    snake_head.setAttribute('class', snake_head.getAttribute('class') + ' snake-unit');  // получили все классы и добавили новый.
    // Тело змейки
...
function move() {
    ...
    if (direction == 'x-') {
        ...
        new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (coord_x + 1))[0];
        if(new_unit == undefined){
            new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (coord_x =0))[0];
        }
    }
    else if (direction == 'y+') {
        ...
        new_unit = document.getElementsByClassName('cell-' + (coord_y + 1) + '-' + (coord_x))[0];
        if(new_unit == undefined){
            new_unit = document.getElementsByClassName('cell-' + (coord_y=0) + '-' + (coord_x ))[0];
        }
...
//////////Проверка на это не бомба/////////////////
function isBombUnit(unit) {//проверка, что змейка не попала в бомбу.
    var check = false;

    if (bombs.includes(unit)) {//если в змейке содержится бомба, значит габэлла...
        check = true;
    }
    return check;
}
//////////Конец проверки что это не бомба////////////
/**
 * Проверка на змейку
 * @param unit
 * @returns {boolean}
...
function finishTheGame() {
    ...
    alert('Вы проиграли!\n Змея врезалась сама в себя!!! Ваш результат: ' + score.toString());
}

function finishTheGame2() {
    gameIsRunning = false;
    clearInterval(snake_timer);
    alert('Вы проиграли!\n Змея съела бомбу!!! Ваш результат: ' + score.toString());
}
